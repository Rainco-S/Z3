...
\[
\begin{split}
    \textbf{con}:&\textbf{CptSync}p(in:(A\mapsto\langle\alpha,a\rangle);out:(B\mapsto\langle\beta,b\rangle))\\
    \textbf{pre}:&\mathcal{D}\langle\alpha,a\rangle\\
    \textbf{post}:&\mathcal{D}\langle\beta,b\rangle\land b=a\land\forall i<|a|.\\
    &((\beta(i).data=\alpha(i).data\land\beta(i).prob=\alpha(i).prob\cdot(1-p))\lor\\
    &(\beta(i).data=corrupted\land\beta(i).prob=\alpha(i).prob\cdot p))
\end{split}
\]
...
\[
\begin{split}
    \textbf{con}:&\textbf{RdmSync}(in:(A\mapsto\langle\alpha,a\rangle);out:(B\mapsto\langle\beta,b\rangle))\\
    \textbf{pre}:&\mathcal{D}\langle\alpha,a\rangle\\
    \textbf{post}:&\mathcal{D}\langle\beta,b\rangle\land b=a\land\forall i<|\beta|.\\
    &((\beta(i).data=0\land\beta(i).prob=1/2)\lor\\
    &(\beta(i).data=1\land\beta(i).prob=1/2))
\end{split}
\]
...
\[
\begin{split}
    \textbf{con}:&\textbf{ProbLossy}q(in:(A\mapsto\langle\alpha,a\rangle);out:(B\mapsto\langle\beta,b\rangle))\\
    \textbf{pre}:&\mathcal{D}\langle\alpha,a\rangle\\
    \textbf{post}:&\mathcal{D}\langle\beta,b\rangle\land PL(\langle\alpha,a\rangle,\langle\beta,b\rangle,q)
\end{split}
\]
where $PL(\langle\alpha,a\rangle,\langle\beta,b\rangle,q)\equiv$
\[
\begin{cases}
    \langle\beta,b\rangle=\langle(),()\rangle & \text{if }\langle\alpha,a\rangle=\langle(),()\rangle\\
    PL(\langle\alpha^{(1)},a^{(1)}\rangle,\langle\beta,b\rangle,q)\lor(PL(\langle\alpha^{(1)},a^{(1)}\rangle,\langle\beta^{(1)},b^{(1)}\rangle,q)\land & \text{else} \\
    |\beta|>0\land\alpha(0).data=\beta(0)\land \beta(0).prob=\alpha(0).prob\cdot(1-q))
\end{cases}
\]
