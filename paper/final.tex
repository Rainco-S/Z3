...
\subsubsection{Sync:}
The synchronous channel \tikz[baseline=-0.8ex]{
    \node[node0, label=left:A] (A) at (0, 0){}; 
    \node[node0, label=right:B] (B) at (1, 0) {}; 
    \chanSync{A}{B}
} transfers the data without delay in time. So it behaves just as the identity function. The pair of I/O operations on its two ends can succeed only simultaneously, and the input is not taken until the output can be delivered, which is captured by the variable $ok$.
\[
\begin{split}
...
On the other hand, for the FIFO1 channel \tikz[baseline=-0.8ex]{
    \node[node0, label=left:A] (A) at (0, 0){}; 
    \node[node0, label=right:B] (B) at (1.5, 0) {}; 
    \chanFifoOneE{A}{B}{e}
} where the buffer contains the data element e, the communication can be initiated only if the data element e can be taken via the sink end. In this case, we have
\[
\begin{split}
    \textbf{con}&:\textbf{FIFO1}(in:(A\mapsto\langle\alpha,a\rangle);out:(B\mapsto\langle\beta,b\rangle))\\
    \textbf{pre}&:\mathcal{D}\langle\alpha,a\rangle\\
...
An exotic channel in Reo is the synchronous drain channel \tikz[baseline=-0.8ex]{
    \node[node0, label=left:A] (A) at (0, 0){}; 
    \node[node0, label=right:B] (B) at (1, 0) {}; 
    \chanSyncDrain{A}{B}
} that has two source ends. Because a drain has no sink end, no data value can ever be obtained from this channel. Thus, all data items written to this channel are lost. A synchronous drain accepts a data item through one of its ends iff a data item is also available for it to simultaneously accept through
...
The lossy synchronous channel \tikz[baseline=-0.8ex]{
    \node[node0, label=left:A] (A) at (0, 0){}; 
    \node[node0, label=right:B] (B) at (1, 0) {}; 
    \chanLossySync{A}{B}
} is similar to a synchronous channel, except that it always accepts all data items through its source end. If it is possible for it to simultaneously dispense the data item through its sink end, the channel transfers the data item.
...
The filter channel \tikz[baseline=-0.8ex]{
    \node[node0, label=left:A] (A) at (0, 0){}; 
    \node[node0, label=right:B] (B) at (1.5, 0) {}; 
    \chanFilterP{A}{B}{p}
...
The p-Producer channel \tikz[baseline=-0.8ex]{
    \node[node0, label=left:A] (A) at (0, 0){}; 
    \node[node0, label=right:B] (B) at (1.5, 0) {}; 
    \chanProducerP{A}{B}{p}
} specifies a producer pattern p which is a set of data values. Once it accepts a data item from the source end, it produces a data item in the set $p$ which is taken synchronously via the sink end.
\[
\begin{split}
...
The asynchronous spout channel \tikz[baseline=-0.8ex]{
    \node[node0, label=left:A] (A) at (0, 0){}; 
    \node[node0, label=right:B] (B) at (1.5, 0) {}; 
    \chanAsynSpout{A}{B}
} outputs two sequences of data items at its two output (sink) ends, but the data items on the two ends are never delivered at the same time.
\[
\begin{split}
...
The \textbf{asynchronous drain} channel \tikz[baseline=-0.8ex]{
    \node[node0, label=left:A] (A) at (0, 0){}; 
    \node[node0, label=right:B] (B) at (1.5, 0) {}; 
    \chanAsynDrain{A}{B}
} inputs two sequences of data items at its two input (source) ends, but the data items on the two ends are never delivered at the same time. Moreover, the \textbf{synchronous spout} channel \tikz[baseline=-0.8ex]{
    \node[node0, label=left:A] (A) at (0, 0){}; 
    \node[node0, label=right:B] (B) at (1.5, 0) {}; 
    \chanSyncSpout{A}{B}
} has two sink ends, and outputs data sequence simultaneously, i.e., the two ends must deliver data items at the same time. Similar to the definition for synchronous drain and asynchronous spout, we can easily derive the design models for asynchronous drain and synchronous spout channels as well, which we skip here.
...
The message-corrupting synchronous channel \tikz[baseline=-0.8ex]{
    \node[node0, label=left:A] (A) at (0, 0){}; 
    \node[node0, label=right:B] (B) at (1.5, 0) {}; 
    \chanCptSync{A}{B}{p}
} is a synchronous channel which has an extra parameter $p$ compared with the primitive synchronous channel. The delivered message can be corrupted with probability $p$. Hence, if a data item flows into the channel through the source end, then the correct data value will be obtained at the sink end with probability $1-p$ and a corrupted data value $\perp$ will be obtained with probability $p$.

\subsubsection{RdmSync:}
The randomized synchronous channel \tikz[baseline=-0.8ex]{
    \node[node0, label=left:A] (A) at (0, 0){}; 
    \node[node0, label=right:B] (B) at (1.5, 0) {}; 
    \chanRdmSync{A}{B}{rand(0, 1)}
} can generate a random number $b\in\{0,1\}$ with equal probability when it is activated through an arbitrary write operation on its source end, and this random number will be taken on the sink end simultaneously.

\subsubsection{ProbLossy:}
The message transmitted by the probabilistic lossy synchronous channel \tikz[baseline=-0.8ex]{
    \node[node0, label=left:A] (A) at (0, 0){}; 
    \node[node0, label=right:B] (B) at (1, 0) {}; 
    \chanProbLossy{A}{B}{q}
} can get lost with a certain probability $q$. It can also act like a \textit{Sync} channel and the message will be delivered successfully with probability $1-q$.

\subsubsection{FtyFIFO1:}
The messages flowing into a faulty FIFO1 channel \tikz[baseline=-0.8ex]{
    \node[node0, label=left:A] (A) at (0, 0){}; 
    \node[node0, label=right:B] (B) at (2, 0) {}; 
    \chanFtyFifoOne{A}{B}{r}
} can get lost with probability $r$ when it is inserted into the buffer. In this case, the buffer remains empty. It can also behave as a normal \textit{FIFO1} channel when the insertion of data into the buffer is successful with probability $1-r$.\par
A mutation of this channel named \textbf{LossyFIFO1} channel \tikz[baseline=-0.8ex]{
    \node[node0, label=left:A] (A) at (0, 0){}; 
    \node[node0, label=right:B] (B) at (2, 0) {}; 
    \chanLossyFifoOne{A}{B}{r}
} works perfectly on the insertion of data item into its buffer but may loose messages from the buffer before being taken on the sink end. The difference between this channel and FtyFIFO1 is that the loss of data items happens in different steps. Loss behavior in this channel arises in the process of being taken from the buffer, while loss behavior in FtyFIFO1 arises in the process of storage into the buffer.
...
The source end of a $t$-timer channal \tikz[baseline=-0.8ex]{
    \node[node0, label=left:A] (A) at (0, 0){}; 
    \node[node0, label=right:B] (B) at (1.5, 0) {}; 
    \chanTimert{A}{B}{t}
} accepts any input value $d$ and returns on its sink end $B$ a timeout signal after a delay of $t$ time units.
\[
\begin{split}
...
The $t$-timer with the $off$-option \tikz[baseline=-0.8ex]{
    \node[node0, label=left:A] (A) at (0, 0){}; 
    \node[node0, label=right:B] (B) at (2, 0) {}; 
    \chanOFFTimert{A}{B}{t}
} allows the timer to be stopped before the expiration of its delay when a special "\textit{off}" value is consumed through its source end.
\[
\begin{split}
...
The reset-timer \tikz[baseline=-0.8ex]{
    \node[node0, label=left:A] (A) at (0, 0){}; 
    \node[node0, label=right:B] (B) at (2, 0) {}; 
    \chanRSTTimert{A}{B}{t}
} allows the timer to be reset to 0 after it has been activated when a special “reset" value is consumed through its source end.
\[
\begin{split}
...
The t-timer with early expiration \tikz[baseline=-0.8ex]{
    \node[node0, label=left:A] (A) at (0, 0){}; 
    \node[node0, label=right:B] (B) at (2, 0) {}; 
    \chanEXPTimert{A}{B}{t}
} makes the timer produce its timeout signal through its sink and reset itself when it consumes a special “expire" value through its source end.
\[
\begin{split}
...
We use test cases to validate the correctness of our method, which can be found at \cite{Z3}, where we provide various basic, timer, and probabilistic connectors along with their mutants to be tested. It's worth mentioning that we develop a template using the \LaTeX package \verb|TikZ| to draw the corresponding graphs of the connectors. The source code of witch is also available in \cite{Z3}. Here we choose three representative examples to show how the system works.
...
c2 = ['Sync A D', 'Sync D E', 'Sync D F', 'Fifo1 E G', 'Fifo1 F H',
      'SyncDrain G H','Sync G B', 'Sync H C']\end{verbatim}
